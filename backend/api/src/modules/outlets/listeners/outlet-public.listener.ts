...
export class OutletPublicListener {
  constructor(private readonly gateway: OutletPublicGateway) {}
  ...
  handleWorkingStatusChanged(event: OutletWorkingStatusChangedEvent): void {
  ...
  handleCameraStatusChanged(event: OutletCameraStatusChangedEvent): void {
  ...
  handleOutletEnabled(event: OutletLifecycleEvent): void {
  ...
  handleOutletDisabled(event: OutletLifecycleEvent): void {
    this.gateway.emitLifecycle({
      outletId: event.outletId,
      status: 'INACTIVE',
    });
