...
export class CategoryPublicGateway implements OnGatewayConnection {
  @WebSocketServer()
  private readonly server: Server;

  constructor(private readonly categoryRepo: CategoryRepository) {
    console.log('🚀 [GATEWAY INIT] CategoryPublicGateway initialized');
  }
  ...
  async handleConnection(client: Socket): Promise<void> {
    console.log('✅ [SOCKET CONNECT] category client connected:', client.id);
  ...
  async emitFullCategories(client?: Socket): Promise<void> {
    ...
    console.log(`📦 [SOCKET PAYLOAD] ${categories.length} categories`);
