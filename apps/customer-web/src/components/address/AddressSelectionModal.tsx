...
import { X, MapPin, Plus, Loader2, Home, Briefcase, Pencil, Trash2, AlertCircle, CheckCircle, Crosshair, Map as MapIcon } from "lucide-react";
      ...
      {popup && (
        <div className="absolute inset-0 z-[10000] flex items-center justify-center bg-black/20 backdrop-blur-[2px]">
          <div className="bg-white p-6 rounded-2xl shadow-2xl max-w-xs w-full text-center border border-slate-100 transform transition-all scale-100">
            ...
            <h3 className="font-bold text-slate-800 mb-2">{popup.type === 'confirm' ? 'Confirm' : popup.type === 'error' ? 'Error' : 'Success'}</h3>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {addresses.map((addr) => (
                <div key={addr.id} className="relative group">
                  <button onClick={() => onSelect(addr)} className="w-full text-left p-4 pr-20 rounded-xl border border-slate-200 hover:border-emerald-500 hover:bg-emerald-50/30 transition-all flex items-start gap-3">
            ...
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <button 
                ...
                className="w-full py-3 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold rounded-xl border border-emerald-200 flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              {/* 4. Detailed Address Inputs */}
              <div className="space-y-4">
                 
                 {/* Row 1: House No & Pincode */}
                 ...
                 <div>
                    ...
                    <div className="relative">
                        ...
                        {details.area.length > 5 && (
                            <div className="absolute bottom-3 right-3 text-xs font-bold flex items-center gap-1 bg-white/90 px-2 py-1 rounded-md shadow-sm border border-slate-100">
                            {geocodingStatus === "SEARCHING" && <><Loader2 size={12} className="animate-spin text-slate-400" /> <span className="text-slate-400">Locating...</span></>}
                            {geocodingStatus === "FOUND" && <><MapIcon size={12} className="text-emerald-500" /> <span className="text-emerald-600">Location Found</span></>}
                            {geocodingStatus === "NOT_FOUND" && <><AlertCircle size={12} className="text-amber-500" /> <span className="text-amber-600">Location not found</span></>}
                            </div>
                        )}
                    </div>
                 </div>
